...
function(t) {
    ...
    Wolmart.ProductList = function() {
        ...
        $.each(JSON.parse(localStorage.getItem('cart')), function(index, item) {
            $('#empty-cart').hide();
            $('#cart-table').show();
            l += `<div class="product product-cart"> <div class="product-detail"> <a href="product-default.html" class="product-name">${item['name']}</a> <div class="price-box"> <span class="product-quantity">${item['quantity']}</span> <span class="product-price">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span> </div> </div> <figure class="product-media"> <a href="product-default.html"> <img src="${item['image'][0]}" alt="product" height="84" width="94" /> </a> </figure> <button class="btn btn-link btn-close" aria-label="button"> <i class="fas fa-times"></i> </button> </div>`
            r += `<div class="product product-cart"> <div class="product-detail"> <h3 class="product-name"> <a href="product-default.html">${item['name']}</a> </h3> <div class="price-box"> <span class="product-quantity">${item['quantity']}</span> <span class="product-price">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span> </div> </div> <figure class="product-media"> <a href="product-default.html"> <img src="${item['image'][0]}" alt="product" height="84" width="94" /> </a> </figure> <button class="btn btn-link btn-close" aria-label="button"> <i class="fas fa-times"></i> </button> </div>`
            q += `<tr> <td class="product-thumbnail"> <div class="p-relative"> <a href="product-default.html"> <figure> <img src="${item['image'][0]}" alt="product" width="300" height="338"> </figure> </a> <button type="submit" class="btn btn-close"><i class="fas fa-times"></i></button> </div> </td> <td class="product-name"> <a href="product-default.html"> ${item['name']} </a> </td> <td class="product-price"><span class="amount">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span></td> <td class="product-quantity"> <div class="input-group"> <input class="quantity form-control" id="${index}" value="${item['quantity']}" type="number" min="1" readonly max="100000"> <button class="quantity-plus w-icon-plus plus"></button> <button class="quantity-minus w-icon-minus minus"></button> </div> </td> <td class="product-subtotal"> <span class="amount" id="amount-${index}">${Wolmart.itemTotalPrice(index)}</span> </td> </tr>`
            s += ` <tr class="bb-no">
                ...
                i.hasClass("disabled") ? alert("Please select some product options before adding this product to your cart.") : (t(".page-wrapper .product img").each((function() {
                    if (this.getAttribute("src") == a.find(".product-media img, .product-image:first-child img").attr("src")) {
                        ...
                        if (e.hasClass('w-icon-cart') || e.hasClass('w-icon-minus')) {
                            ...
                            e.removeClass('btn-cart'), e.attr('href', (_,attr)=>inner == '<i class="w-icon-cart"></i> Add To Cart' ? '#' : '/')
                        }
                    }
                }
                )),
