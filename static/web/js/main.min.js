...
localStorage.setItem('getUser', JSON.stringify(false));
...
$('.review-form').on('submit', function (e) {
    ...
    $.ajax({
        ...
        success: function (e) {
            if (e['product']) {
                $('#product-tab-reviews').html(e['form'])
            } else {
                let comment = "<div class=\"user-wrap\"><div class=\"user-photo\"> <figure><img src=\"/static/web/images/vendor/wcfm/avatar.png\" alt=\"Avatar\"\n" + "   width=\"113\" height=\"112\"/></figure><div class=\"rated text-center\"> <label>Rated</label> <span class=\"score\">5.0</span></div>\n" + "    </div> <div class=\"user-info\"><h4 class=\"user-name\">John Doe</h4><div class=\"user-date mb-7\">\n" + "                                                    <span>1 Reviews</span>\n" + "                                                    <span>April 1, 2021 12:12 Pm</span>\n" + "                                                </div>\n" + "                                                <p>Diam in arcu cursus euismod quis. Eget sit amet tellusvitae\n" + "                                                    sapien pellentesque habitant morbi tristique senectus et.\n" + "                                                    Cras adipiscing enim ermentum et sollicitudin ac orci phasellus.\n" + "                                                </p>\n" + "                                            </div>\n" + "                                            <!-- End of User Info -->\n" + "                                            <div class=\"review-ratings\">\n" + "                                                <div class=\"ratings-container\">\n" + "                                                    <div class=\"ratings-full\"><span class=\"ratings\" style=\"width: 100%;\"></span>\n" + "</div>\n" + "                                                    <label>5.0 Feature</label>\n" + "                                                </div>\n" + "                                            </div>\n" + "                                        </div>";
                document.querySelector('.review-ratings') && document.querySelector('.review-ratings').insertAdjacentHTML('afterend', comment)
...
function f() {
    ...
    Wolmart.ProductList(), Wolmart.RefreshPage();
}

let currency = {
    currency: 'NGN',
    pattern: '! #,##0.00',
    selector: '.new-price, .old-price, .price, .product-price',
    valueOnError: '₦5,000'
};

const getUser = () => {
    $('.user-in').each(function () {
        $(this).hide()
    })
    $('.btn-wishlist').each(function () {
        $(this).removeClass('w-icon-heart').removeClass('btn-product-icon')
    })
    fetch('/api/consumer', {
        headers: {
        ...
        if (data.user) {
            ...
            localStorage.setItem('getUser', JSON.stringify(true))
            $('.btn-wishlist').each((function () {
                let i = $(this), a = i.closest(".product");
                $(this).addClass('btn-product-icon').addClass('w-icon-heart')
                $.each(data['wish_list'], function (q, item) {
                    if (item.id === Number($(a).attr('id'))) {
                        $(i).removeClass("w-icon-heart").addClass("w-icon-heart-full")
                    }
                })
            }))
            document.querySelectorAll('.user-in').forEach(item => {
                item.style.display = "flex"
            })
            $('a.user-ino').each(function (i, e) {
                $(e).removeClass('sign-in')
                $(e).attr('href', '/account')
            })
            document.querySelectorAll('.lastname').forEach(name => {
                name.value = `${data.user['last_name']}`
            })
            document.querySelectorAll('.firstname').forEach(name => {
                name.value = `${data.user['first_name']}`
            })
            document.querySelectorAll('.user-out').forEach(item => {
                item.style.display = 'none'
            })
            document.querySelectorAll('.first-last').forEach(name => {
                name.innerText = data.user['first_name'] + ' ' + data.user['last_name']
            })
            document.querySelectorAll('.email').forEach(name => {
                name.innerText = data.user['email']
            })
            document.querySelectorAll('.login-popin').forEach(form => {
                form.remove()
            })

            document.querySelector('#wishlist') && renderData(data['wish_list'], 'wishlist')
        }
    });
}
getUser()

function accessUser(data, i) {
    localStorage.setItem('token', data['access'])
    if (JSON.stringify(localStorage.getItem('cart')).length !== 0) {
        alert("Your cart will be synced back into your already available cart, Don't worry item won't be saved twice, your current cart will take prioty")
    }
    fetch('/api/consumer', {
        method: "POST", headers: {
            "Content-Type": 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}`
        }, body: JSON.stringify({
            'action': 'loggin', 'cart': cart
        })
    }).then(r => {
        getUser()
        ...
        setTimeout((function () {
            Wolmart.Minipopup.open({
                ...
                name: 'Welcome Back',
                nameLink: '/account',
                imageSrc: '/static/web/images/shop/1.jpg',
                imageLink: '/account',
                message: 'message',
                actionTemplate: '<a href="/account" class="btn btn-rounded btn-sm">View Acount</a>'
            })
        }), 1000)
...
$.extend($.easing, {
    ...
    Wolmart.$window = t(window), Wolmart.$body = t(document.body), Wolmart.status = "", Wolmart.isIE = navigator.userAgent.indexOf("Trident") >= 0, Wolmart.isEdge = navigator.userAgent.indexOf("Edge") >= 0, Wolmart.isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), Wolmart.call = function (t, e) {
        ...
        , $('.hs-toggle').hover(function () {
        let i = $(this)
        i.toggleClass('show')
        $('.search-form.form-control').focus(function () {
            i.addClass('show')
        })
    }), Wolmart.ProductList = function () {
        let btnCompare = $('.btn-compare');
        $.each(btnCompare, function (e, element) {
            let q = element.closest('.product, .product-popup')
            $.each(JSON.parse(Wolmart.getCookie('compare')), function (i, value) {
                let p = value['id']
                if ($(q).attr('id') == p) $(element).addClass('w-icon-check-solid').addClass('added').removeClass('w-icon-compare').attr("href", "/compare")
            })
        })

    }, Wolmart.RefreshPage = function () {
        ...
        let categoried = "", categorymobile = "", categoriesmenu = "", l = "", r = "", q = "", s = "";
        ;fetch('/api/category').then(res => res.json()).then(data => {
            $.each(data, function (index, item) {
                ...
                <a href="/product/product-category/${item.title}">
                ...
                <ul>
                    <li>
                        <a href="#">Women</a>
                        <ul>
                            <li><a href="/product/product-category/${item.title}">New Arrivals</a>
                            </li>
                        </ul>
                    </li>
                    <li>
            ...
            <a href="/product/product-category/${item.title}">
                ...
                categoriesmenu += item.category_list ? `<li><a href="/product/product-category/${item.title}"><i class="${item.icon}"></i>${item.title}</a>
                    <ul class="megamenu">${categroytree(item.category_list)}</ul></li>` : `<li><a href="/product/product-category/${item.title}"><i class="${item.icon}"></i>${item.title}</a></li>`
            })
            $('.filter-category').prepend(categoried)
            $('.category-menu').html(categoriesmenu)
            $('#categorymobile').prepend(categorymobile)
        })
        $.each(JSON.parse(localStorage.getItem('cart')), function (index, item) {
            $('#empty-cart').hide();
            $('#cart-table').show();
            l += `<div class="product product-cart"> <div class="product-detail"> <a href="/product/${item['id']}" class="product-name">${item['name']}</a> <div class="price-box"> <span class="product-quantity">${item['quantity']}</span> <span class="product-price">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span> </div> </div> <figure class="product-media"> <a href="/product/${item['id']}"> <img src="${item['image'][0]}" alt="product" height="84" width="94" /> </a> </figure> <button class="btn btn-link btn-close" aria-label="button"> <i class="fas fa-times"></i> </button> </div>`
            r += `<div class="product product-cart"> <div class="product-detail"> <h3 class="product-name"> <a href="/product/${item['id']}">${item['name']}</a> </h3> <div class="price-box"> <span class="product-quantity">${item['quantity']}</span> <span class="product-price">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span> </div> </div> <figure class="product-media"> <a href="/product/${item['id']}"> <img src="${item['image'][0]}" alt="product" height="84" width="94" /> </a> </figure> <button class="btn btn-link btn-close" aria-label="button"> <i class="fas fa-times"></i> </button> </div>`
            q += `<tr> <td class="product-thumbnail"> <div class="p-relative"> <a href="/product/${item['id']}"> <figure> <img src="${item['image'][0]}" alt="product" width="300" height="338"> </figure> </a> <button type="submit" class="btn btn-close"><i class="fas fa-times"></i></button> </div> </td> <td class="product-name"> <a href="/product/${item['id']}"> ${item['name']} </a> </td> <td class="product-price"><span class="amount">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span></td> <td class="product-quantity"> <div class="input-group"> <input class="quantity form-control" id="${index}" value="${item['quantity']}" type="number" min="1" readonly max="100000"> <button class="quantity-plus w-icon-plus plus"></button> <button class="quantity-minus w-icon-minus minus"></button> </div> </td> <td class="product-subtotal"> <span class="amount" id="amount-${index}">${Wolmart.itemTotalPrice(index)}</span> </td> </tr>`
            s += ` <tr class="bb-no">
                <td class="product-name">${item['name']} <i
                        class="fas fa-times"></i> <span
                        class="product-quantity">${item['quantity']}</span></td>
                <td class="product-total">${Wolmart.itemTotalPrice(index)}</td>
            </tr>`
        })
        $('.cart-body').html(q)
        $('#checkout-body').html(s)
        $('.cart-view').html(l)
        document.querySelectorAll('.cart-view-mobile').forEach(cart => cart.innerHTML = r)
        if (r !== "") {
            $('.empty-msg').remove()
            document.querySelectorAll('.cart-view, .cart-view-mobile').forEach(cart => cart.innerHTML += `<div class="cart-total"> <label>Subtotal:</label> <span class="price totalcartprice">${Wolmart.TotalCartPrice()}</span> </div> <div class="cart-action" style="margin-bottom:5rem;"> <a href="/cart" class="btn btn-dark btn-outline btn-rounded">View Cart</a> <a href="/checkout" class="btn btn-primary btn-rounded">Checkout</a> </div> <div class="cart-total"> </div>`)
        } else {
            !document.querySelector('.empty-msg') && document.querySelectorAll('.cart-view').forEach(cart => cart.insertAdjacentHTML('beforebegin', '<p class="woocommerce-mini-cart__empty-message empty-msg">No products in the cart.</p>'))
        }
        document.querySelectorAll('.cart-count').forEach(counter => counter.innerHTML = Wolmart.totalItem())
        document.querySelectorAll('.totalcartprice').forEach(counter => counter.innerHTML = Wolmart.TotalCartPrice())
    }
        , Wolmart.parseOptions = function (t) {
        return "string" == typeof t ? JSON.parse(t.replace(/'/g, '"').replace(";", "")) : {}
    }
        , Wolmart.getURLParams = function (url) {
    ...
    Wolmart.parseTemplate = function (t, e) {
        ...
        , Wolmart.TotalCartPrice = function (t) {
        let cost = 0;
        $.each(JSON.parse(localStorage.getItem('cart')), function (index, item) {
            cost += (item.price * item.quantity)
        })
        return OSREC.CurrencyFormatter.format(cost, currency)
    }
        , Wolmart.itemTotalPrice = function (n) {
        let product = JSON.parse(localStorage.getItem('cart'))[n],
            total = Number(product.quantity) * Number(product.price);
        return OSREC.CurrencyFormatter.format(total, currency)
    }
        ...
        var a = new Date;
        a.setTime(a.getTime() + (i * 24 * 60 * 60 * 1e3)), document.cookie = t + "=" + e + ";expires=" + a.toUTCString() + ";path=/"
    }
        ...
        Wolmart.$body.on('click', '.icon-mode-grid, .icon-mode-list', (function (e) {
            var i = t(e.currentTarget);
            if (e.preventDefault(), !i.hasClass('active')) {
                let v = t(i)[0].classList[0].split('-')[2]
                i.prev().removeClass('active'), i.next().removeClass('active'), i.addClass('active');
                $(`#shop-${v}`).prev('.product-wrapper').hide(), $(`#shop-${v}`).next('.product-wrapper').hide(), $(`#shop-${v}`).show()
            }
        }))
    }, Wolmart.$body.on('click', 'a[href^=product]', function (e) {
        var i = $(this)
        Wolmart.setCookie('details', $(i.closest('.product, product-popup')).attr('id'), 0.02)
    })
        ...
        minWidth: 992, maxWidth: 2e4, top: !1, hide: !1, scrollMode: !0
    }, Wolmart.stickyToolboxOptions = {
        minWidth: 0, maxWidth: 767, top: !1, scrollMode: !0
    }, Wolmart.stickyProductOptions = {
        minWidth: 0, maxWidth: 2e4, scrollMode: !0, top: !1, hide: !1
        ...
        return e.prototype.init = function (e, i) {
            ...
            i > this.top && this.isWrap ? (this.height = a[0].offsetHeight, a.hasClass("fixed") || a.parent().css("height", this.height + "px"), a.hasClass("fix-top") ? (a.css("margin-top", e.offsetTop + "px"), this.zIndex = this.options.max_index - e.index) : a.hasClass("fix-bottom") ? (a.css("margin-bottom", e.offsetBottom + "px"), this.zIndex = this.options.max_index - e.index) : a.css({
                transition: "opacity .5s", "z-index": this.zIndex
                ...
                "margin-top": "", "margin-bottom": "", "z-index": ""
        ...
        itemsSelector: ".grid-item", layoutMode: "masonry", percentPosition: !0, masonry: {
        ...
        name: "fadeIn", duration: "1.2s", delay: ".2s"
        ...
        responsive: !0, borderSize: 0, zoomType: "inner", onZoomIn: !0, magnify: 1.1
        ...
        Wolmart.$body.hasClass("home") && !Wolmart.getCookie("hideNewsletterPopup") && setTimeout((function () {
            Wolmart.popup({
                items: {
                    src: ".newsletter-popup"
                }, type: "inline", tLoading: "", mainClass: "mfp-newsletter mfp-fadein-popup", callbacks: {
            ...
            e.preventDefault(), Wolmart.popup({
                items: {
                    src: '<video src="' + t(e.currentTarget).attr("href") + '" autoplay loop controls>', type: "inline"
    ...
    function SearchCart(id) {
        for (let x in JSON.parse(localStorage.getItem('cart'))) {
            let item = JSON.parse(localStorage.getItem('cart'))[x];
            if (`${item.id}` === id) return x
        }
        return false
    }

    if (!Wolmart.getCookie('compare')) Wolmart.setCookie('compare', JSON.stringify([]), 4);
    ...
    var e = {
        init: function () {
            ...
            Wolmart.call(Wolmart.ratingTooltip, 500), Wolmart.call(Wolmart.setProgressBar(".progress-bar"), 500), this.initVariation(), this.initProductsScrollLoad(".scroll-load"), Wolmart.$body.on("mousedown", ".select-menu", (function (e) {
            ...
            })), Wolmart.$body.on("click", ".product:not(.product-select) .btn-cart, .product-popup .btn-cart, .home .product-single .btn-cart, .btn-cart", (function (e) {
                e.preventDefault();
                var i = $(this), a = i.closest(".product, .product-popup, tr"), index = SearchCart(`${a.attr('id')}`),
                    message = index ? "<p>has been removed from cart </p>" : "<p>has been added to cart:</p>",
                    inner = index ? '<i class="w-icon-cart"></i> Add To Cart' : '<i class="w-icon-cart"></i> Select Options';
                // Todo: add disabled add to cart button
                if (i.hasClass("disabled")) {
                    alert("Please select some product options before adding this product to your cart.")
                } else {
                    i.addClass("load-more-overlay loading")
                    if (JSON.parse(localStorage.getItem('getUser'))) {
                        fetch(`/api/consumer`, {
                            method: "POST", headers: {
                                "Content-Type": 'application/json',
                                'Authorization': `Bearer ${localStorage.getItem('token')}`
                            }, body: JSON.stringify({
                                'action': 'cart', 'product': `${a[0].id}`
                            })
                        ...
                        if (index) {
                            ...
                            fetch(`/api/product/${a[0].id}`).then(res => res.json()).then(data => {
                                data['quantity'] = a.find(".quantity").val() ? a.find(".quantity").val() : 1;
                                cart.push(data);
                                f()
                            })
                        }
                    }
                    (t(".page-wrapper .product, .product-popup").each((function () {

                        if (this.getAttribute("id") === a.attr("id")) {
                            var e = t(this).closest(".product").find(".btn-cart")
                            if (e.hasClass('w-icon-cart') || e.hasClass('w-icon-minus')) {
                                e.length && (e.toggleClass("w-icon-cart").toggleClass("w-icon-minus").toggleClass('added')) && e.attr("title", (_, attr) => attr === 'remove from cart' ? 'add to cart' : 'remove from cart')
                            } else {
                                e.html(inner), t(this).closest('.product').toggleClass('product-select'), e.attr('href', (_, attr) => inner === '<i class="w-icon-cart"></i> Add To Cart' ? '#' : 'product')
                            }
                        }
                    })), setTimeout((function () {
                        Wolmart.Minipopup.open({
                ...
                e.preventDefault();
                let i = t(this), a = i.closest(".product, .product-popup");
                if (JSON.parse(localStorage.getItem('getUser'))) {
                    fetch(`/api/consumer`, {
                        method: "POST", headers: {
                            "Content-Type": 'application/json',
                            ...
                            'action': 'wishlist', 'product': `${$(a).attr('id')}`
                        })
                    })
                    i.toggleClass("added")
                    t(".product").each((function () {
                        if (this.getAttribute('id') === $(a).attr('id')) {
                            let k = $(this).find('.btn-wishlist')
                            setTimeout((function () {
                                k.toggleClass("w-icon-heart").toggleClass("w-icon-heart-full")
                            }), 500)
                        }
                    }))
                }
            })), //TODO : change compare and wishlist button
            (e = t(".product-popup")).length && Wolmart.$body.on("click", ".btn-quickview", (function (i) {
                let b = $(this), a = b.closest('.product, tr'), v = $('.product-popup');
                b.addClass("load-more-overlay loading"), v.attr('id', a.attr('id'))
                v.find(".product-title").text(a.find('.product-name, product-title').text()), v.find('.product-link-wrapper').html(Icon(Number(a.attr('id')), 'compare'));
                i.preventDefault(), fetch(`/api/product/${a.attr('id')}`).then(res => res.json()).then(data => {
                    let i = "", x = "";
                    $.each(data.image, function (index, image) {
                        i += '<div class="swiper-slide"><figure class="product-image"> <img src="' + image + '" data-zoom-image="' + image + '" alt="' + data.name + '" width="800" height="900"> </figure> </div>';
                        x += `<div class="product-thumb swiper-slide"><img src="${image}" alt="${data.name} Thumbnail" width="103" height="116"></div>`
                    })
                    v.find('.product-category > a').text(data.category);
                    v.find('.product-price').text(OSREC.CurrencyFormatter.format(data['price'], currency))
                    document.querySelector('.popup-thumbnail').innerHTML = x;
                    $('.popup-product').html(i);
                    setTimeout((function () {
                        b.removeClass("load-more-overlay loading"), Wolmart.popup({
                            items: {
                                src: e[0].outerHTML
                            }, callbacks: {
                                open: function () {
                ...
                let e, i = JSON.parse(Wolmart.getCookie('compare')), a = t(".page-wrapper > .compare-popup");

                function q(removed) {
                    let r = `${removed['id']}`;
                    r && t(".page-wrapper .product").each((function () {
                        if (this.getAttribute("id") === r) {
                            var e = t(this).closest(".product").find(".btn-compare");
                            e.length && (e.removeClass("added").attr("href", "#"), e.removeClass("w-icon-check-solid").addClass("w-icon-compare"))
                        }
                    }))
                }
                ...
                a.length || document.body.classList.contains("docs") || (t(".page-wrapper").append('<div class="compare-popup">                    <div class="container">                        <div class="compare-title">                            <h4 class="title title-center">Compare Products</h4>                        </div>                        <ul class="compare-product-list list-style-none">                            <li></li><li></li><li></li><li></li>                        </ul>                        <a href="#" class="btn btn-clean">Clean All</a>                        <a href="compare" class="btn btn-dark btn-rounded">Start Compare !</a>                    </div>                </div>                <div class="compare-popup-overlay">                </div>'), a = t(".page-wrapper > .compare-popup")), Wolmart.$body.on("click", ".product .btn-compare, .product-popup .btn-compare", (function (o) {
                    var s = t(this), r = {};
                    s.hasClass("added") && returne(), o.preventDefault(), s.toggleClass("added"), setTimeout((function () {
                        t(".page-wrapper .product img, product-wrapper .product-wrap img, .product-popup").each((function () {
                            if (this.getAttribute("src") === s.closest(".product").find("img").eq(0).attr("src") || this.getAttribute('id') === s.closest('.product').attr('id')) {
                                var e = t(this).closest(".product").find(".btn-compare");
                                e.length && (e.removeClass("w-icon-compare").addClass("w-icon-check-solid"), e.attr("href", "/compare"), e.addClass('added'))
                            }
                        }))
                        a.addClass("show")
                    }), 500);
                    fetch(`/api/product/${s.closest('.product')[0].id}`).then(r => r.json()).then(data => {
                        delete data.description, console.log(data), i.length >= 4 && q(i.shift()), i.push(data) && Wolmart.setCookie('compare', JSON.stringify(i), 4), t(".compare-popup li").each((function (t) {
                            i[t] && (this.innerHTML = '<a href="/product/' + i[t]['id'] + '"><figure><img src="' + i[t]['image'][0] + '"/></figure></a>                                        <a href="#" class="btn btn-remove"><i class="w-icon-times-solid"></i></a>')
                        })), e = i.length, n()
                    })
                    ...
                    $.each(i, function (index, item) {
                        r && t(".page-wrapper .product img, .product-popup").each((function () {
                    ...
                    })), i.splice(0, 4), e = i.length, t(this).parent().find(".compare-product-list li").empty(), n(), Wolmart.setCookie('compare', JSON.stringify(i))
    ...
    async function itemQuantity(id, event) {
        let product = cart[id];
        product.quantity = event
        cart[id] = product;
        localStorage.setItem('cart', JSON.stringify(cart))
        document.getElementById(`amount-${id}`).innerText = Wolmart.itemTotalPrice(id);
        window.onload();
    }
    ...
    e.min = 1, e.max = 1e6, e.value = 1, e.prototype.init = function (t) {
        ...
        var i = this;
        i.$minus = !1, i.$plus = !1, i.$value = !1, i.value = !1, i.startIncrease = i.startIncrease.bind(i), i.startDecrease = i.startDecrease.bind(i), i.stop = i.stop.bind(i), i.min = parseInt(t.attr("min")), i.max = parseInt(t.attr("max")), i.min || t.attr("min", i.min = e.min), i.max || t.attr("max", i.max = e.max), i.$value = t.val(i.value = t[0].value), i.$minus = t.parent().find(".quantity-minus").on("mousedown", (function (t) {
        ...
        t && t.preventDefault();
        var e = this;
        e.value = e.$value.val(), e.value < e.max && e.$value.val(++e.value), itemQuantity(e.$value[0].id, e.$value[0].value), console.log(this)
        ...
        t && t.preventDefault();
        var e = this;
        e.value = e.$value.val(), e.value > e.min && e.$value.val(--e.value), itemQuantity(e.$value[0].id, e.$value[0].value), e.decreaseTimer = Wolmart.requestTimeout((function () {
    ...
    let compare = JSON.parse(Wolmart.getCookie('compare'));
    for (let index in compare) {
        $('#empty-compare').hide();
        $('.compare-table').show();
        let item = compare[index];
        document.querySelector('.compare-products') && document.querySelector('.compare-products').insertAdjacentHTML("beforeend", ` <div class="compare-col compare-product"> <a href="#" class="btn remove-product"><i class="w-icon-times-solid"></i></a> <div class="product text-center" id="${item['id']}"> <figure class="product-media"> <a href="/product/${item['id']}"> <img src="${item["image"][0]}" alt="Product" width="228" height="257" /> </a> <div class="product-action-vertical"> <a href="#" class="btn-product-icon btn-cart w-icon-cart"></a> <a href="#" class="btn-product-icon btn-wishlist w-icon-heart"></a> </div> </figure> <div class="product-details"> <h3 class="product-name"><a href="/product/${item['id']}">${item["name"]}</a> </h3> </div> </div> </div>`)
        document.querySelector('.compare-price') && document.querySelector('.compare-price').insertAdjacentHTML("beforeend", `<div class="compare-col compare-value"> <div class="product-price"> <span class="new-price">${OSREC.CurrencyFormatter.format(item['price'], currency)}</span> </div> </div>`)
    }
}(jQuery), function (t) {
    function e(t, e) {
        return this.init(t, e)
    ...
    e.defaults = {
        ...
        quickview: {
            type: "inline", mainClass: "mfp-product mfp-fade", tLoading: "Loading..."
        }, video: {
            type: "iframe", mainClass: "mfp-fade", preloader: !1, closeBtnInside: !1
        }, login: {
            type: "ajax", mainClass: "mfp-login-popup mfp-fade ", tLoading: "", preloader: !1
        }
    ...
    }, $(document).on('click', '#user-up', function (e) {
        e.preventDefault();
        let email = $('#email_1').val(), password = $('#password_1').val(), password2 = $('#password_1').val();
        let i = $(this)[0];
        $(i).addClass("load-more-overlay loading")
    ...
    $(document).on('click', '#user-in', function (e) {
        e.preventDefault();
        let email = $('#username').val(), password = $('#password').val();
        let i = $(this)[0];
        $(i).addClass("load-more-overlay loading")
        fetch('/api/token/', {
            method: 'POST', headers: {
    ...
    var i = function () {
        ...
        var i = t(this);
        i.hasClass("added") || (e.preventDefault(), setTimeout((function () {
            i.toggleClass("w-icon-heart").toggleClass("w-icon-heart-full").addClass("added").attr("href", "wishlist.html")
        }), 500))
    }, n = function (e) {
    ...
    function e(e) {
        ...
        if ((i = n.closest(".review-image").length ? n.closest(".review-image").find("img") : o.find(".product-single-swiper").length ? o.find(".product-single-swiper .swiper-slide:not(.cloned) img:first-child") : o.find(".product-gallery-carousel").length ? o.find(".product-gallery-carousel .swiper-slide:not(.cloned) img") : o.find(".product-image img:first-child")).length) {
            a = i.map((function () {
                var e = t(this);
                return {
                    src: e.attr("data-zoom-image"), w: 800, h: 900, title: e.attr("alt")
                }
            })).get();
            ...
            if ("undefined" != typeof PhotoSwipe) {
                ...
                if ("rtl" == Wolmart.$body.attr("dir")) var d = new PhotoSwipe(c, PhotoSwipeUI_Default, a, {
                    index: l, closeOnScroll: !1, showAnimationDuration: 0, rtl: !0
                }); else d = new PhotoSwipe(c, PhotoSwipeUI_Default, a, {
                    index: l, closeOnScroll: !1, showAnimationDuration: 0
                });
    ...
    function i(t) {
        t.preventDefault(), Wolmart.popup({
            items: {
                src: '<video src="assets/video/memory-of-a-woman.mp4" autoplay loop controls>', type: "inline"
            }, mainClass: "mfp-video-popup"
        }, "video")
    }
    ...
    function n(e) {
        ...
        if (i.hasClass("disabled")) alert("Please select some product options before adding this product to your cart."); else {
            if (a.length) a.fadeOut((function () {
                ...
                var n = '<div class="alert alert-success alert-cart-product mb-2">                            <a href="/cart" class="btn btn-success btn-rounded">View Cart</a>                            <p class="mb-0 ls-normal">“' + i.closest(".product-single").find(".product-title").text() + '” has been added to your cart.</p>                            <a href="#" class="btn btn-link btn-close" aria-label="button"><i class="close-icon"></i></a>                            </div>';
    ...
    Wolmart.initProductSinglePage = function () {
        t(".product-gallery").each((function () {
            ...
            e.preventDefault(e), t.fn.ThreeSixty && function (t) {
                t.preventDefault(), Wolmart.popup({
                    type: "inline", mainClass: "product-popupbox wm-fade product-360-popup", preloader: !1, items: {
            ...
            if (i.hasClass("show-code-added")) {
                ...
                i[0].removeChild(a), t(".code-popup #textareaCode").text(i.data("codepopup")), t(".code-copy a").html('<i class="copy-icon far fa-copy"></i>Copy to Clipboard'), Wolmart.$body.on("click", ".code-copy a", (function (e) {
                    ...
                    Wolmart.popup({
                        items: {
                            src: ".code-popup"
                        }, type: "inline", tLoading: "", mainClass: "mfp-code mfp-fadein-popup"
    ...
    Wolmart.appearAnimate(".appear-animate"), Wolmart.setTab(".nav-tabs"), Wolmart.stickyContent(".sticky-header", {
        ...
        minWidth: 0, maxWidth: 767, top: 150, hide: !0, max_index: 2100, scrollMode: !0
    ...
    Wolmart.canvasWidth = window.innerWidth, Wolmart.resizeTimeStamp = 0, Wolmart.resizeChanged = !1, Wolmart.status = "loaded", document.body.classList.add("loaded"), Wolmart.ProductList(), Wolmart.RefreshPage(), Wolmart.call(Wolmart.initLayout), Wolmart.call(Wolmart.init), Wolmart.status = "complete", Wolmart.$window.trigger("wolmart_complete")
};
