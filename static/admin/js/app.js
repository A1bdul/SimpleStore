...
$('button[data-bs-target]').click(function () {
    let url = $(this).closest('tr').attr('id');
    fetch(`/api/cart/${url}`)
        .then(r => r.json())
        .then(data => {
            let template = ''
            for (let i in data.products) {
                ...
                template += '<tr><th scope="row"><div><img src="'+
                item.image[0]+'" alt="" class=" avatar-sm"></div></th><td><div><h5 class="text-truncate font-size-14">'+
                item.name+'</h5><p class="text-muted mb-0">₦ '+item.price +' x '+ item.quantity+'</p></div></td><td>'+ item.price * item.quantity+'</td></tr>'
            }
            template += '<tr><td colspan="2"><h6 class="m-0 text-right">Shipping:</h6></td><td>Free</td></tr><tr><td colspan="2"><h6 class="m-0 text-right">Total:</h6></td>\n' +
                '<td> ₦' + data.price + '</td></tr>';
            $('#order-detail-table').html(template)
        })
})
